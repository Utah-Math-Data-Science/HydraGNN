...
from torch_geometric.nn.models.dimenet import BesselBasisLayer, SphericalBasisLayer, ResidualLayer
...
class DIMEStack(Base):
    ...
    def _conv_args(self, data):
        ...
        assert data.pos is not None, 'DimeNet requires node positions (data.pos) to be set.'
...
class Interaction(torch.nn.Module):
    ...
    def forward(self,
        ...
        x_kj = torch.einsum('wj,wl,ijl->wi', spherical_basis, x_kj[edge_index_from], self.W)
        x_kj = scatter(x_kj, edge_index_to, dim=0, dim_size=x.size(0), reduce='sum') # message passing

        x_ji = self.act(self.lin_to(x))
        h = x_ji + x_kj # aggregates my learned message and my from messages to the next neighbor

        for layer in self.layers_before_skip: # this added resnet is not actually doing any message passing and is an interesting addition
            h = layer(h)
        h = self.act(self.lin(h)) + x # incorporates a residual connection to the input feature
